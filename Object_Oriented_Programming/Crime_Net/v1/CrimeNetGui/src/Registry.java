...
public class Registry {
	...
	public ArrayList<Suspect> getListOfSuspects() {
	...
	public ArrayList<SMS> getMessages(String number) {
		ArrayList<SMS> smsList = new ArrayList<>();
		for(Communication com: listOfCommunications) {
			if(com instanceof SMS) {
				smsList.add((SMS) com);
			}
		}
		ArrayList<SMS> selectedSMSList = new ArrayList<>();
		for(SMS sms: smsList) {
			if(number.equals(sms.getPhoneNumber1()) || (number.equals(sms.getPhoneNumber2())))
				if(sms.getSMSContent().contains("Bomb") || 
						sms.getSMSContent().contains("Attack") || 
						sms.getSMSContent().contains("Explosives") || 
						sms.getSMSContent().contains("Gun"))
					selectedSMSList.add(sms);
		}
		return selectedSMSList;
	}
}
